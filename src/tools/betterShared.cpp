...
#include <functional>
#include "SpinLock.h"
#include "PlaysBetterWithOthers.h"
using namespace std;
...
int main(int argc, char * argv[]) {
  ...
  for (int i = 0; i < thread_count; ++i)
    threads[i] = new thread(&PlaysBetterWithOthers::run, PlaysBetterWithOthers(),
                            ref(spin), ref(the_counter), times_to_increment);
